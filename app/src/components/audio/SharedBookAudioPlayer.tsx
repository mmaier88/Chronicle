...
export function SharedBookAudioPlayer({ bookTitle, sections, shareToken }: SharedBookAudioPlayerProps) {
  ...
  // Fetch audio for a section using shared endpoint
  const fetchAudio = useCallback(async (sectionId: string): Promise<{ url: string; duration: number } | null> => {
    ...
    try {
      // Use shared TTS endpoint with token
      const response = await fetch(`/api/tts/shared/${shareToken}/section/${sectionId}`)
