  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-serif font-semibold text-amber-900">{typedBook.title}</h1>
          <div className="flex items-center gap-4 mt-2">
            <span className={`text-xs px-2 py-1 rounded-full ${
              typedBook.status === 'drafting' ? 'bg-amber-100 text-amber-800' :
          ...
          {typedBook.core_question && (
            <p className="text-amber-700/80 mt-3 max-w-2xl">{typedBook.core_question}</p>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white/80 rounded-lg border border-amber-200/60 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-amber-900">Chapters</h2>
            ...
            {!typedBook.constitution_locked ? (
              <div className="text-center py-8 border border-dashed border-amber-300/60 rounded-lg">
                ...
                <p className="text-amber-800 mb-2">Lock the Constitution to add chapters</p>
                <p className="text-sm text-amber-600/70">The Constitution must be finalized before writing begins</p>
              </div>
            ) : typedChapters.length === 0 ? (
              <div className="text-center py-8 border border-dashed border-amber-300/60 rounded-lg">
                <BookOpen className="w-8 h-8 text-amber-400 mx-auto mb-3" />
                <p className="text-amber-800 mb-2">No chapters yet</p>
                <p className="text-sm text-amber-600/70 mb-4">Start structuring your book by adding chapters</p>
              ...
              <div className="space-y-3">
                {typedChapters.map((chapter, idx) => (
                  <Link
                    key={chapter.id}
                    href={`/vibe/books/${typedBook.id}/chapters/${chapter.id}`}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="text-xs text-amber-600/70">Chapter {idx + 1}</span>
                        <h3 className="font-medium text-amber-900">{chapter.title}</h3>
                        {chapter.purpose && (
                          <p className="text-sm text-amber-700/70 mt-1 line-clamp-2">{chapter.purpose}</p>
                        )}
                      </div>
                      <span className={`text-xs px-2 py-1 rounded-full ${
